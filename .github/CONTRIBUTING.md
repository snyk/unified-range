...
*Important:* when fixing a bug, please commit a **failing test** first so that Travis CI (or we can) can show the code failing. Once that commit is in place, then commit the bug fix, so that we can test *before* and *after*.
